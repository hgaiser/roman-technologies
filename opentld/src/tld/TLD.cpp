...
void TLD::selectObject(Mat img, Rect * bb) {
	...
	detectorCascade->init(img, bb);

	//Init particle filter
	//particleFilter->init(bb);
...
void TLD::processImage(Mat img) {
	...
	if(detectorEnabled && (!alternating || medianFlowTracker->trackerBB == NULL)) {
		detectorCascade->detect(grey_frame, img);
	}
...
//	Rect* trackerBB = particleFilter->trackerBB;
...
void TLD::initialLearning() {
	...
	detectorCascade->detect(currImg, currRGB);
	detectorCascade->templateDetector->init(currRGB, *currBB);
