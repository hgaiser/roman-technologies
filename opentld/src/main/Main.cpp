	...
	while(imAcqHasMoreFrames(imAcq)) {
		...
		if(showOutput || saveDir != NULL) {
			...
			if(showOutput) {
				...
				if(key == 't') {
					tld->detectorCascade->templateDetector->mEnabled = !tld->detectorCascade->templateDetector->mEnabled;
					printf("templateDetector: %d\n", tld->detectorCascade->templateDetector->mEnabled);
				}
